...
public class NFController {
	...
	private static final byte FGSERVE = 2;
	private static final byte BGSERVE = 3;
	...
	private int port; // Puerto TCP en el que escucha el peer
	...
	public void processCommand() throws IOException {
		...
		case NFCommands.COM_USERLIST:
			 ...
			 */
			commandSucceeded = controllerDir.getAndPrintUserList(directory);
			break;
		...
		case NFCommands.COM_PUBLISH:
			 ...
			 */
			commandSucceeded = controllerDir.publishLocalFiles(port, directory);
			break;
		...
		case NFCommands.COM_DOWNLOADFROM:
			...
			commandSucceeded = controllerPeer.downloadFileFromSingleServer(downloadTargetServer, downloadTargetFileHash,
	...
	public boolean canProcessCommandInCurrentState() {
		...
		case NFCommands.COM_LOGOUT:
			if (currentState != LOGGED_IN) {
				...
				if (currentState == BGSERVE) {
					System.err.println("* You cannot logout beacause you have launch a server");
				}
				else {
		...
		case NFCommands.COM_USERLIST:
			if (currentState != LOGGED_IN && currentState != BGSERVE) {
				commandAllowed = false;
				System.err.println("* You cannot receive the userlist because you are not logged in from the directory");
			}
			break;
		case NFCommands.COM_FGSERVE:
			if (currentState != LOGGED_IN) {
				commandAllowed = false;
				System.err.println("* You cannot launch a server because you are not logged in from the directory");
			}
			break;
		case NFCommands.COM_BGSERVE:
			if (currentState != LOGGED_IN) {
		...
		case NFCommands.COM_DOWNLOADFROM:
			if (currentState != LOGGED_IN && currentState != BGSERVE) {
				commandAllowed = false;
				System.err.println("* You cannot download from a peer beacuse you are not logged in from the directory");
			}
			break;
		default:
