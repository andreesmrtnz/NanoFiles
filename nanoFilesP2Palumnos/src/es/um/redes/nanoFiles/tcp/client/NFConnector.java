...
public class NFConnector {
	...
	public boolean downloadFile(String targetFileHashSubstr, File file) throws IOException {
		 ...
		 */
		PeerMessage message = new PeerMessage(PeerMessageOps.OPCODE_DOWNLOADFROM);

		message.setFilehash(targetFileHashSubstr.getBytes());
		message.setTamanoHash(targetFileHashSubstr.length());
		...
		if (respuesta.getOpcode() == PeerMessageOps.OPCODE_FILEMESSAGE) {
			if (!file.exists()) {
				System.out.println("Descargado");
				file.createNewFile();
				FileOutputStream fos = new FileOutputStream(file);
				fos.write(respuesta.getData());
				fos.close();
			}
		}
		/*
		 ...
		 */
		String hash = FileDigest.getChecksumHexString(FileDigest.computeFileChecksum(file.getPath()));
